...
class TSPDataLoader(object):
    ...
    def _create_input_queue(self, queue_capacity_factor=16):
        ...
        for name in self.data_num.keys():
            ...
            self.queue_ops[name] = tf.RandomShuffleQueue(
                ...
                shapes=[[self.max_length, self.dim, ], [self.max_length]],
    ...
    def _maybe_generate_and_save(self, except_list=[]):
        ...
        for name, num in self.data_num.items():
            ...
            if not os.path.exists(path):
                ...
                x = np.zeros([num, self.max_length, self.dim], dtype=np.float32)
    ...
    def read_zip_and_update_data(self, path, name):
        ...
        x = np.zeros([len(x_list), self.max_length, self.dim], dtype=np.float32)
...
class CVRPDataLoader(TSPDataLoader):
    def __init__(self, config, data_format='preproc', rng=None):
    ...
    def _maybe_generate_and_save(self, except_list=[]):
        ...
        for name, num in self.data_num.items():
            ...
            if not os.path.exists(path):
                ...
                np.savez(path, x=x, y=y)
                self.data[name] = CVRP(x=x, y=y, name=name)
            else:
                tf.logging.info("Skip creating {} for [{}]".format(path, self.task))
                tmp = np.load(path)
                self.data[name] = CVRP(x=tmp['x'], y=tmp['y'], name=name)
    ...
    def download_google_drive_file(self):
        paths = {'test': 'data/cvrp_sep_test.txt', 'train': 'data/cvrp_sep_train.txt'}
        """
        paths = {}
        for mode in ['train', 'test']:
            candidates = []
    ...
    def read_zip_and_update_data(self, path, name):
        ...
        x_list, y_list = self.read_dataset(paths, self.max_length)
    ...
    def read_dataset(self, paths, max_length):
        x, y = [], []
        for path in paths:
            tf.logging.info("Read dataset {} which is used in the paper..".format(path))
            #length = max(re.findall('\d+', path))
